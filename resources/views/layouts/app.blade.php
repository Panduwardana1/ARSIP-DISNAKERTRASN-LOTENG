...
<body class="antialiased bg-slate-100 text-slate-900 min-h-screen">
    ...
    <main class="ml-[15rem] h-screen max-h-screen w-auto bg-white selection:bg-amber-600 selection:text-white">
        <div class="flex items-center justify-between px-4 py-6 h-[4rem]">
            ...
            <div class="flex space-x-2 items-center font-inter">
                <span
                    class="inline-flex items-center rounded-md bg-amber-400/10 px-2 py-1.5 text-xs font-medium text-amber-700 inset-ring ring-amber-500 ring-1 inset-ring-red-400/20">{{ date('D-M-Y') }}</span>
                <div class="flex items-center gap-2 p-1.5 text-zinc-700">
        ...
        {{-- Konten main --}}
        <div class="bg-white p-2 border-t h-screen">
            @yield('content')
        </div>
    </main>
