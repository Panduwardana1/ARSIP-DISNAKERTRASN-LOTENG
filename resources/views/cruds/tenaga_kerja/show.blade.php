...
@section('content')
    <div class="h-full overflow-y-auto bg-slate-50 px-6 py-6">
        <div class="mx-auto max-w-5xl space-y-6">
            ...
            @endif
            @php
                $riwayatRekap = collect($riwayatRekap ?? $tenagaKerja->rekaps ?? []);
            @endphp
                        ...
                        <dl class="mt-4 space-y-3 text-sm text-zinc-600">
                            <div class="flex items-start justify-between">
                                ...
                                <dd class="text-right text-zinc-700">
                                    {{ optional($tenagaKerja->created_at)->translatedFormat('d M Y H:i') ?? '-' }}
                                </dd>
                            </div>
                            <div class="flex items-start justify-between">
                                <dt class="text-zinc-500">Pembaharuan</dt>
                                <dd class="text-right text-zinc-700">
                                    {{ optional($tenagaKerja->updated_at)->translatedFormat('d M Y H:i') ?? '-' }}
                                </dd>
                            </div>
                        </dl>
                    </section>
