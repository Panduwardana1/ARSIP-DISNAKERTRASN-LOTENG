...
@section('pageTitle', 'SIREKAP - PASMI | Disnakertrans')
@section('titleContent', 'Edit Data P3MI')
    ...
    <div class="h-full overflow-y-auto py-4">
        <div class="mx-auto max-w-full space-y-6 px-2 font-inter">
            <div class="flex flex-col justify-between gap-4 md:flex-row md:items-start">
                <div class="space-y-1">
                    <h2 class="text-xl font-semibold text-zinc-800">Formulir P3MI</h2>
                    <p class="text-sm text-zinc-500">Lengkapi profil P3MI penempatan tenaga kerja untuk digunakan pada
                        modul SIREKAP.</p>
                </div>
            ...
            <form action="{{ route('sirekap.perusahaan.update', $perusahaan) }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="space-y-10 p-6 md:p-10">
                    ...
                    <section class="space-y-6">
                        <div>
                            ...
                            <p class="text-sm text-zinc-500">Informasi komunikasi yang dapat dihubungi beserta identitas
                                visual.</p>
                        </div>
                        <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                            <label class="block text-sm font-medium text-zinc-700">
                            ...
                            <label class="block text-sm font-medium text-zinc-700">
                                Nomor HP
                                <input type="text" name="nomor_hp" value="{{ old('nomor_hp', $perusahaan->nomor_hp) }}"
                                    class="mt-1 w-full rounded-md border border-zinc-200 bg-white px-4 py-2.5 text-sm text-zinc-700 transition focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/30">
                                @error('nomor_hp')
                                    <span class="mt-1 block text-xs text-rose-600">{{ $message }}</span>
                                @enderror
                            ...
                            <label class="md:col-span-2 block text-sm font-medium text-zinc-700">
                                ...
                                <textarea name="alamat" rows="3"
                                    class="mt-1 w-full rounded-md border border-zinc-200 bg-white px-4 py-2.5 text-sm text-zinc-700 transition focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/30">{{ old('alamat', $perusahaan->alamat) }}</textarea>
                                @error('alamat')
                                    <span class="mt-1 block text-xs text-rose-600">{{ $message }}</span>
                                @enderror
                            </label>
                            {{-- todo image --}}
                            <label class="grid text-sm font-medium text-zinc-700">
                                Upload Logo (opsional)
                                <input type="file" name="gambar"
                                    class="mt-1 max-w-[18rem] rounded-md border border-zinc-200 bg-white px-3 py-2 text-sm text-zinc-700 transition file:mr-2 file:rounded-md file:border-0 file:bg-zinc-100 file:px-3 file:py-1 file:text-sm file:font-medium file:text-zinc-700 hover:file:bg-zinc-200 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/30">
                                <span class="mt-1 block text-xs text-zinc-400">Format gambar (JPG, PNG) maksimal 2
                                    MB.</span>
                                @if ($perusahaan->gambar)
                                    <div class="mt-3 flex items-center gap-3 text-xs text-zinc-500">
                                        <img src="{{ Storage::url($perusahaan->gambar) }}" alt="Logo {{ $perusahaan->nama }}"
                                            class="h-10 w-10 rounded-md border border-zinc-200 object-cover"
                                            onerror="this.style.display='none'">
                                        <span>Logo saat ini akan tetap digunakan jika tidak memilih file baru.</span>
                                    </div>
                                @endif
                                @error('gambar')
                                    <span class="mt-1 block text-xs text-rose-600">{{ $message }}</span>
