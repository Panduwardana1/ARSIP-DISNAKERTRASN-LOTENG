...
@section('headerActions')
    <a href="{{ route('disnakertrans.pendidikan.create') }}"
        ...
        <div class="mx-auto w-full max-w-4xl space-y-6">
            ...
            <div class="rounded-xl bg-white p-6 shadow-sm ring-1 ring-zinc-200">
                <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    <form action="{{ route('disnakertrans.pendidikan.index') }}" method="GET" class="w-full md:w-auto">
                        <div class="flex items-center gap-2">
                            <input type="search" name="search" placeholder="Cari kode, nama, atau level"
                        ...
                        <tbody class="divide-y divide-zinc-200">
                            @forelse ($pendidikan as $item)
                                <tr class="hover:bg-zinc-50">
                                    <td class="px-4 py-3">
                                        {{ $pendidikan->firstItem() + $loop->index }}
                                    </td>
                                    ...
                                    <td class="px-4 py-3 text-center">
                                        <div class="flex items-center justify-center gap-2">
                                            ...
                                            {{-- <x-modal-confirm-delete :action="route('disnakertrans.pendidikan.destroy', $item)">
                                                <button type="button"
                ...
                @if ($pendidikan->hasPages())
                    <div class="mt-6 flex flex-col items-center justify-between gap-3 text-sm text-zinc-500 md:flex-row">
                        <div>
                            Menampilkan
                            <span class="font-semibold text-zinc-700">{{ $pendidikan->firstItem() }}</span>
                            hingga
                            <span class="font-semibold text-zinc-700">{{ $pendidikan->lastItem() }}</span>
                            dari
                            <span class="font-semibold text-zinc-700">{{ $pendidikan->total() }}</span>
                            data
                        </div>
                        {{ $pendidikan->onEachSide(1)->links() }}
                    </div>
                @endif
            </div>
        </div>
