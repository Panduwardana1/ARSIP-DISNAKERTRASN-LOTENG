...
@section('titlePageContent', 'Ubah Pendidikan')
...
@section('headerActions')
    <div class="flex items-center gap-2">
        ...
        <a href="{{ route('disnakertrans.pendidikan.show', $pendidikan) }}"
...
@section('content')
    <div class="max-w-3xl mx-auto w-full px-4 py-6">
        <div class="rounded-xl bg-white p-6 shadow-sm ring-1 ring-zinc-200">
            <h2 class="text-lg font-semibold text-zinc-800">Perbarui Data Pendidikan</h2>
            <p class="mt-1 text-sm text-zinc-500">Ubah informasi pendidikan lalu simpan perubahan.</p>
            ...
            <form action="{{ route('disnakertrans.pendidikan.update', $pendidikan) }}" method="POST"
                class="mt-6 space-y-6">
                @csrf
                @method('PUT')

                @include('cruds.pendidikan.partials.form', ['submitLabel' => 'Simpan Perubahan'])
            </form>
        </div>
    </div>
@endsection
