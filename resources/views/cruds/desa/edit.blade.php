        ...
        <form action="{{ route('sirekap.desa.update', $desa) }}" method="POST" class="space-y-6">
            ...
            <div class="grid gap-6 md:grid-cols-2">
                <div>
                    ...
                    <input type="text" name="nama" id="nama"
                        ...
                        value="{{ old('nama', $desa->nama) }}" required>
                    @error('nama')
                    ...
                    <select name="kecamatan_id" id="kecamatan_id"
                        ...
                        @foreach ($kecamatans as $kecamatan)
                            <option value="{{ $kecamatan->id }}"
                                @selected(old('kecamatan_id', $desa->kecamatan_id) == $kecamatan->id)>
                                {{ $kecamatan->nama }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div>
                ...
                <select name="tipe" id="tipe"
                    ...
                    <option value="desa" @selected(old('tipe', $desa->tipe) === 'desa')>Desa</option>
                    <option value="kelurahan" @selected(old('tipe', $desa->tipe) === 'kelurahan')>Kelurahan</option>
                </select>
                @error('tipe')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
