...
@section('titlePageContent', 'Ubah Agency')
                ...
                >
                    ...
                    @foreach ($perusahaans as $perusahaan)
                        <option value="{{ $perusahaan->id }}"
                            @selected(old('perusahaan_id', $agency->perusahaan_id) == $perusahaan->id)>
                            {{ $perusahaan->nama }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="grid gap-6 md:grid-cols-2">
                <div>
                    ...
                    <input
                        ...
                        name="lowongan"
                        id="lowongan"
                        value="{{ old('lowongan', $agency->lowongan) }}"
                        maxlength="100"
                        class="mt-1 w-full rounded-md border border-zinc-300 px-3 py-2 text-sm text-zinc-800 focus:border-indigo-500 focus:ring-indigo-500"
                    >
                    @error('lowongan')
                ...
                <div>
                    ...
                    <textarea
                        ...
                        id="keterangan"
                        rows="4"
                        placeholder="Masukkan catatan penting mengenai agency."
                        class="mt-1 w-full rounded-md border border-zinc-300 px-3 py-2 text-sm text-zinc-800 focus:border-indigo-500 focus:ring-indigo-500">{{ old('keterangan', $agency->keterangan) }}</textarea>
                    @error('keterangan')
                        <p class="mt-1 text-sm text-rose-600">{{ $message }}</p>
                    @enderror
                </div>
