...
@section('titlePageContent', 'Daftar Lowongan')
    ...
    <div class="flex h-full flex-col font-inter">
        ...
        <main class="flex-1 px-4 py-6">
            <div class="flex items-center justify-between pb-4 w-full">
                {{-- search --}}
                <form method="GET" action="{{ route('sirekap.lowongan.index') }}" class="relative w-full max-w-md"
                    role="search">
                                    ...
                                    @foreach ($lowongans as $lowongan)
                                        @php
                                            $statusStyles = [
                                                ...
                                                'non_aktif' => [
                                                    'badge' =>
                                                        'inline-flex items-center gap-2 rounded-full bg-rose-100 px-3 py-1 text-xs font-semibold text-rose-700',
                                        ...
                                        <tr class="transition hover:bg-zinc-50/70">
                                            <td class="px-6 py-4 align-top">
                                                ...
                                                <div class="text-xs text-zinc-500">Dibuat
                                                    {{ $lowongan->created_at?->translatedFormat('d M Y') ?? '-' }}</div>
                                            </td>
                                            <td class="px-6 py-4 align-top">
                                                <div class="text-sm text-zinc-700">{{ $lowongan->agensi->nama ?? '-' }}
                                                </div>
                                            </td>
                                            <td class="px-6 py-4 align-top">
                                                <div class="text-sm text-zinc-700">{{ $lowongan->perusahaan->nama ?? '-' }}
                                                </div>
                                            </td>
                                            <td class="px-6 py-4 align-top">
                                                <div class="text-sm text-zinc-700">{{ $lowongan->destinasi->nama ?? '-' }}
                                                </div>
                                            </td>
                                            <td class="px-6 py-4 align-top">
                            ...
                            @foreach ($lowongans as $lowongan)
                                @php
                                    $statusStyles = [
                                        ...
                                        'non_aktif' => [
                                            'badge' =>
                                                'inline-flex items-center gap-2 rounded-full bg-rose-100 px-3 py-1 text-[11px] font-semibold text-rose-700',
                                ...
                                <article class="rounded-xl border border-zinc-200 p-4 shadow-sm">
                                    <div class="flex items-start justify-between gap-3">
                                        <div>
                                            ...
                                            <p class="text-xs text-zinc-500">Dibuat
                                                {{ $lowongan->created_at?->translatedFormat('d M Y') ?? '-' }}</p>
                                        </div>
                                        <span class="{{ $statusClass['badge'] }}">
                                            <span class="{{ $statusClass['dot'] }}"></span>
                                            {{ ucfirst(str_replace('_', ' ', $lowongan->is_aktif)) }}
                                    ...
                                    <dl class="mt-4 space-y-3 text-sm text-zinc-600">
                                        <div>
                                            ...
                                            <dd class="mt-1 font-medium text-zinc-700">{{ $lowongan->agensi->nama ?? '-' }}
                                            </dd>
                                        </div>
                                        <div>
                                            <dt class="text-xs uppercase tracking-wide text-zinc-400">Perusahaan</dt>
                                            <dd class="mt-1 font-medium text-zinc-700">
                                                {{ $lowongan->perusahaan->nama ?? '-' }}</dd>
                                        </div>
                                        <div>
                                            <dt class="text-xs uppercase tracking-wide text-zinc-400">Destinasi</dt>
                                            <dd class="mt-1 font-medium text-zinc-700">
                                                {{ $lowongan->destinasi->nama ?? '-' }}</dd>
                                        </div>
                                    </dl>
