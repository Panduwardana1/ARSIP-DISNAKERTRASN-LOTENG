...
@section('pageTitle', 'SIREKAP - PASMI | Lowongan | Ubah')
@section('titleContent', 'Ubah Lowongan')
    ...
    <div class="h-full overflow-y-auto py-4">
        <div class="mx-auto max-w-3xl space-y-6 px-2 font-inter">
            <div class="flex flex-col justify-between gap-4 md:flex-row md:items-start">
                <div class="space-y-1">
                    <h2 class="text-xl font-semibold text-zinc-800">Edit Lowongan</h2>
                    <p class="text-sm text-zinc-500">Perbarui detail lowongan kerja untuk memastikan data tetap akurat.</p>
                </div>
            ...
            <form action="{{ route('sirekap.lowongan.update', $lowongan) }}" method="POST"
                    ...
                    <section class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 gap-5">
                            <label class="block text-sm font-medium text-zinc-700">
                                ...
                                <input type="text" name="nama" value="{{ old('nama', $lowongan->nama) }}" maxlength="150"
                                    required
                        ...
                        <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                            <label class="block text-sm font-medium text-zinc-700">
                                ...
                                <select name="agensi_id" required
                                    ...
                                    @foreach ($daftarAgensi as $id => $nama)
                                        <option value="{{ $id }}" @selected(old('agensi_id', $lowongan->agensi_id) == $id)>{{ $nama }}</option>
                                    @endforeach
                                </select>
                                @error('agensi_id')
                                    <span class="mt-1 block text-xs text-rose-600">{{ $message }}</span>
                            ...
                            <label class="block text-sm font-medium text-zinc-700">
                                ...
                                <select name="perusahaan_id" required
                                    ...
                                    @foreach ($daftarPerusahaan as $id => $nama)
                                        <option value="{{ $id }}" @selected(old('perusahaan_id', $lowongan->perusahaan_id) == $id)>
                                            {{ $nama }}</option>
                                    @endforeach
                                </select>
                                @error('perusahaan_id')
                            ...
                            <label class="block text-sm font-medium text-zinc-700">
                                ...
                                <select name="destinasi_id" required
                                    ...
                                    @foreach ($daftarDestinasi as $id => $nama)
                                        <option value="{{ $id }}" @selected(old('destinasi_id', $lowongan->destinasi_id) == $id)>
                                            {{ $nama }}</option>
                                    @endforeach
                                </select>
                                @error('destinasi_id')
                            ...
                            <label class="block text-sm font-medium text-zinc-700">
                                ...
                                <select name="is_aktif" required
                                    ...
                                    @foreach ($statusOptions as $value => $label)
                                        <option value="{{ $value }}" @selected(old('is_aktif', $lowongan->is_aktif) === $value)>
                                            {{ $label }}</option>
                                    @endforeach
                                </select>
                                @error('is_aktif')
                            ...
                            <label class="block text-sm font-medium text-zinc-700 md:col-span-2">
                                ...
                                <textarea name="catatan" rows="3" maxlength="255"
                                    class="mt-1 w-full rounded-md border border-zinc-200 bg-white px-4 py-2.5 text-sm text-zinc-700 transition focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/30">{{ old('catatan', $lowongan->catatan) }}</textarea>
                                @error('catatan')
                                    <span class="mt-1 block text-xs text-rose-600">{{ $message }}</span>
                                @enderror
                            </label>
