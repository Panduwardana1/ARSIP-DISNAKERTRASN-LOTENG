...
@section('pageTitle', 'Sirekap Pasmi | Negara')
@section('titlePageContent', 'Data Negara Tujuan')
@section('description', 'Kelola data negara tujuan penempatan.')

@section('headerAction')
    <div class="flex flex-col sm:flex-row gap-3 items-center justify-between w-full">
        {{-- Search Bar --}}
        <form method="GET" action="{{ route('sirekap.negara.index') }}" class="relative w-full sm:max-w-xs font-inter group">
            <span class="absolute inset-y-0 left-3 flex items-center text-zinc-400 group-focus-within:text-blue-600">
                <x-heroicon-o-magnifying-glass class="w-5 h-5" />
            </span>
            <input type="search" name="q" value="{{ $search ?? '' }}" placeholder="Cari negara"
        ...
        <div class="w-full sm:w-auto" x-data>
            <button type="button" @click="$dispatch('negara-modal:create')"
...
@section('content')
    ...
    <div x-data="negaraModal({ items: @js($modalNegara), oldForm: @js($oldFormData) })" x-init="init()"
        x-on:negara-modal:create.window="openCreate()" class="space-y-4">
        {{-- Main Table Card --}}
                        ...
                        <tr class="group hover:bg-zinc-50/80 transition-colors duration-200">
                            ...
                            <td class="p-4 align-middle text-center">
                                <span class="text-sm text-zinc-500">
                                    {{ ($negara->currentPage() - 1) * $negara->perPage() + $loop->iteration }}
                                </span>
                            </td>
                            ...
                            <td class="p-4 align-middle text-end">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <x-modal-delete :action="route('sirekap.negara.destroy', $items)" :title="'Hapus Data'"
                        ...
                        <tr>
                            <td colspan="4" class="p-12 text-center">
                                <div class="flex flex-col items-center justify-center text-zinc-500">
                                    <x-heroicon-o-globe-alt class="w-12 h-12 text-zinc-300 mb-3" />
                                    <p class="text-base font-medium">Belum ada data negara</p>
    ...
    <div x-cloak x-show="open" x-transition.opacity.duration.200ms x-on:keydown.escape.window="close()"
        ...
        <div x-show="open" x-transition.scale.duration.200ms
            ...
            <div class="flex items-start justify-between border-b px-5 py-4">
                <div class="space-y-0.5">
                    ...
                    <h2 class="text-lg font-semibold text-zinc-900"
                        x-text="mode === 'create' ? 'Tambah Negara' : 'Ubah Data Negara'"></h2>
                    <p class="text-sm text-zinc-500">Kelola data negara langsung di halaman ini.</p>
                </div>
            ...
            <form :action="formAction" method="POST" class="space-y-5 p-5">
                ...
                <input type="hidden" name="negara_id" :value="form.id">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" id="nama" name="nama" x-model="form.nama" maxlength="100" required
                        class="mt-1 w-full rounded-md border border-zinc-300 px-3 py-2 focus:outline-none focus:border-emerald-400"
    ...
    <script>
        function negaraModal(config) {
            return {
                ...
                updateBase: '{{ url('/sirekap/negara') }}',
                get formAction() {
                    return this.mode === 'create'
                        ? '{{ route('sirekap.negara.store') }}'
