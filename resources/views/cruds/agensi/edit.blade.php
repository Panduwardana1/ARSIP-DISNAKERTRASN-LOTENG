...
@section('titlePageContent', 'Ubah Agensi')
...
@section('headerActions')
    <div class="flex items-center gap-2">
        ...
        <a href="{{ route('disnakertrans.agensi.show', $agensi) }}"
...
@section('content')
    <div class="max-w-3xl mx-auto w-full px-4 py-6">
        <div class="rounded-xl bg-white p-6 shadow-sm ring-1 ring-zinc-200">
            <h2 class="text-lg font-semibold text-zinc-800">Perbarui Data Agensi</h2>
            <p class="mt-1 text-sm text-zinc-500">Perbarui informasi agensi kemudian simpan perubahan.</p>
            ...
            <form action="{{ route('disnakertrans.agensi.update', $agensi) }}" method="POST" class="mt-6 space-y-6">
                @csrf
                @method('PUT')

                @include('cruds.agensi.partials.form', ['submitLabel' => 'Simpan Perubahan'])
            </form>
        </div>
    </div>
@endsection
