...
@section('titlePageContent', 'Data Kecamatan')
@section('description', 'Kelola data kecamatan dan jumlah desa terkait.')

@section('headerAction')
    <div class="flex flex-col sm:flex-row gap-3 items-center justify-between w-full">
        {{-- Search Bar --}}
        <form method="GET" action="{{ route('sirekap.kecamatan.index') }}" class="relative w-full sm:max-w-xs font-inter group">
            <span class="absolute inset-y-0 left-3 flex items-center text-zinc-400 group-focus-within:text-blue-600">
                <x-heroicon-o-magnifying-glass class="w-5 h-5" />
            </span>
            <input type="search" name="q" value="{{ $search ?? '' }}" placeholder="Cari kecamatan"
        ...
        <div class="w-full sm:w-auto" x-data>
            <button type="button" @click="$dispatch('kecamatan-modal:create')"
...
@section('content')
    @php
        ...
        $modalKecamatans = $kecamatans
            ->map(fn($item) => ['id' => $item->id, 'nama' => $item->nama])
            ->values();
    @endphp

    <div x-data="kecamatanModal({ items: @js($modalKecamatans), oldForm: @js($oldFormData) })" x-init="init()"
        x-on:kecamatan-modal:create.window="openCreate()" class="space-y-4">
    {{-- Error Alert --}}
    @error('app')
                        ...
                        <tr class="group hover:bg-zinc-50/80 transition-colors duration-200">
                            ...
                            <td class="p-4 align-middle text-end">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <x-modal-delete :action="route('sirekap.kecamatan.destroy', $items)" :title="'Hapus Data'" :message="'Data Kecamatan ' .
                                        $items->nama .
                                        ' akan dihapus beserta data desa didalamnya.'"
                        ...
                        <tr>
                            <td colspan="5" class="p-12 text-center">
                                <div class="flex flex-col items-center justify-center text-zinc-500">
                                    <x-heroicon-o-map class="w-12 h-12 text-zinc-300 mb-3" />
                                    <p class="text-base font-medium">Belum ada data kecamatan</p>
    ...
    <div x-cloak x-show="open" x-transition.opacity.duration.200ms x-on:keydown.escape.window="close()"
        ...
        <div x-show="open" x-transition.scale.duration.200ms
            ...
            <div class="flex items-start justify-between border-b px-5 py-4">
                <div class="space-y-0.5">
                    ...
                    <h2 class="text-lg font-semibold text-zinc-900"
                        x-text="mode === 'create' ? 'Tambah Data Kecamatan' : 'Ubah Data Kecamatan'"></h2>
                    <p class="text-sm text-zinc-500">Kelola data kecamatan langsung di halaman ini.</p>
                </div>
            ...
            <form :action="formAction" method="POST" class="space-y-5 p-5">
                ...
                <input type="hidden" name="kecamatan_id" :value="form.id">
...
@push('scripts')
    <script>
        function kecamatanModal(config) {
            return {
                open: false,
                mode: 'create',
                form: {
                ...
                updateBase: '{{ url('/sirekap/kecamatan') }}',
                get formAction() {
                    return this.mode === 'create'
                        ? '{{ route('sirekap.kecamatan.store') }}'
