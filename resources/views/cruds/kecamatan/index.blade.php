    ...
    <div class="flex h-full flex-col font-inter">
        <main class="flex-1 px-4 py-4">
            <header class="flex items-center justify-start pb-6">
                <h2 class="font-semibold text-3xl">Kecamatan</h2>
            </header>
            <div class="flex flex-col gap-3 pb-4 md:flex-row md:items-center md:justify-between">
                <form method="GET" action="{{ route('sirekap.kecamatan.index') }}" class="relative w-full max-w-md"
                    role="search">
                    ...
                    <input id="search" name="q" type="search" placeholder="Cari nama atau kode kecamatan"
                        value="{{ request('q') }}"
                    ...
                    @if (filled(request('q')))
                        <a href="{{ route('sirekap.kecamatan.index', request()->except('q', 'page')) }}"
                                        ...
                                        <tr class="transition hover:bg-zinc-50/70">
                                            <td class="px-6 py-4 align-top">
                                                <div class="mt-1 text-xs text-zinc-500">
                                                    {{ $kecamatans instanceof \Illuminate\Contracts\Pagination\Paginator ? $kecamatans->firstItem() + $loop->index : $loop->iteration }}
                                                </div>
                                            </td>
                                            <td class="px-6 py-4 align-top">
                                            ...
                                            <td class="px-6 py-4 align-top">
                                                <span class="text-sm font-medium uppercase text-zinc-800">
                                                    {{ $kecamatan->kode }}
                                                </span>
                                            </td>
                                            <td class="px-6 py-4 align-top">
                                                <div class="text-xs text-zinc-500">
                                                    {{ $kecamatan->created_at?->translatedFormat('d M Y') ?? '-' }}
                                                </div>
                                            </td>
                                            <td class="px-6 py-4 align-top">
                                                <div class="flex items-center justify-end gap-2">
                                                    <a href="{{ route('sirekap.kecamatan.edit', $kecamatan) }}"
                                                        ...
                                                        aria-label="Ubah data {{ $kecamatan->nama }}">
                                                        <x-heroicon-o-pencil-square class="h-4 w-4" />
                                                    </a>
                                                    <x-modal-delete :action="route('sirekap.kecamatan.destroy', $kecamatan)" title="Hapus Kecamatan"
                                    ...
                                    <header class="flex items-start justify-between gap-3">
                                        <div class="flex items-center gap-3">
                                            <div
                                                ...
                                                <x-heroicon-o-map class="h-6 w-6 text-zinc-400" />
                                            </div>
                                            <div>
                                                <h3 class="text-base font-semibold text-zinc-900">{{ $kecamatan->nama }}
                                                </h3>
                                                <p class="text-xs text-zinc-500">
                                                    Kode: {{ strtoupper($kecamatan->kode) }}
                                                </p>
                                            </div>
                                        </div>
                                        <div class="flex items-center gap-2">
                                            <a href="{{ route('sirekap.kecamatan.edit', $kecamatan) }}"
                                                ...
                                                aria-label="Ubah {{ $kecamatan->nama }}">
                                                <x-heroicon-o-pencil-square class="h-4 w-4" />
                                            </a>
                                        </div>
                                    </header>
                ...
                @if ($kecamatans instanceof \Illuminate\Pagination\LengthAwarePaginator)
                    <div
                        ...
                        <div class="text-center sm:text-left">
                            Menampilkan {{ $kecamatans->firstItem() ?? 0 }} - {{ $kecamatans->lastItem() ?? 0 }} dari
                            {{ $kecamatans->total() }} Kecamatan
                        </div>
                        {{-- <div class="flex justify-center sm:justify-end">
                            {{ $kecamatans->withQueryString()->links('components.pagination.simple') }}
                        </div> --}}
                    </div>
                @endif
            </div>
