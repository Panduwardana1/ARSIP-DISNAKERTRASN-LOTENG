...
@section('titlePageContent', 'Daftar Kecamatan')

@section('content')
    <section class="container mx-auto space-y-6 px-4">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div class="flex items-center gap-x-3">
                <h2 class="text-lg font-semibold text-zinc-900">Data Kecamatan</h2>
                <span class="rounded-full bg-emerald-50 px-3 py-1 text-xs font-medium text-emerald-600">
                    {{ $kecamatans instanceof \Illuminate\Contracts\Pagination\LengthAwarePaginator ? $kecamatans->total() : $kecamatans->count() }}
                    wilayah
                </span>
            </div>

            <div class="flex w-full flex-col gap-3 sm:w-auto sm:flex-row sm:items-center">
                <form method="GET" action="{{ route('sirekap.kecamatan.index') }}" class="flex flex-1 items-center gap-2 sm:flex-none">
                    ...
                    <label for="search-kecamatan" class="relative flex-1 sm:w-64">
                        <span class="sr-only">Cari Kecamatan</span>
                        <input
                            type="search"
                            id="search-kecamatan"
                            name="q"
                ...
                @if (filled(request('q')))
                    <a
                        href="{{ route('sirekap.kecamatan.index') }}"
                ...
                <a
                    href="{{ route('sirekap.kecamatan.create') }}"
                        ...
                        <table class="min-w-full divide-y divide-zinc-200">
                            <thead class="bg-slate-800">
                                <tr>
                                    ...
                                    <th scope="col" class="px-4 py-3.5 text-left text-sm font-semibold text-white">Kode</th>
                                    <th scope="col" class="px-4 py-3.5 text-center text-sm font-semibold text-white">
                                        <x-heroicon-o-map class="mr-1 inline h-4 w-4 text-zinc-200" />
                                        Desa
                                    </th>
                                ...
                                @forelse ($kecamatans as $kecamatan)
                                    <tr class="hover:bg-zinc-50/70">
                                        <td class="px-4 py-4 text-sm text-zinc-700">
                                            <div class="font-semibold text-zinc-900">{{ $kecamatan->nama }}</div>
                                            <p class="mt-1 text-xs text-zinc-500">
                                                Dibuat {{ $kecamatan->created_at?->translatedFormat('d M Y') ?? 'tidak diketahui' }}
                                            </p>
                                        </td>
                                        <td class="px-4 py-4 text-sm font-mono text-zinc-700">
                                            {{ $kecamatan->kode ?? '-' }}
                                        </td>
                                        <td class="px-4 py-4 text-center text-sm font-semibold text-indigo-600">
                                            {{ $kecamatan->desas_count ?? 0 }}
                                        </td>
                                        <td class="px-4 py-4">
                                            <div class="flex flex-wrap items-center gap-2">
                                                <a
                                                    href="{{ route('sirekap.kecamatan.edit', $kecamatan) }}"
                                                ...
                                                <x-modal-delete
                                                    :action="route('sirekap.kecamatan.destroy', $kecamatan)"
                                                    :title="'Delete ' . $kecamatan->nama"
                                                    :message="'Kecamatan ' . $kecamatan->nama . ' akan dihapus setelah dipastikan tidak memiliki data desa.'"
                                                    confirm-text="Hapus Kecamatan"
                                                >
                                                    <button
                                                        type="button"
                                ...
                                @empty
                                    <tr>
                                        <td colspan="4" class="px-4 py-6 text-center text-sm text-zinc-500">
                                            Belum ada data kecamatan. <a href="{{ route('sirekap.kecamatan.create') }}" class="font-medium text-indigo-600 underline">Tambah sekarang</a>
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
        ...
        @if ($kecamatans instanceof \Illuminate\Contracts\Pagination\LengthAwarePaginator)
            <div class="flex flex-col gap-2 border-t pb-8 border-zinc-200 pt-4 text-sm text-zinc-500 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    Menampilkan {{ $kecamatans->firstItem() ?? 0 }} - {{ $kecamatans->lastItem() ?? 0 }} dari {{ $kecamatans->total() }} kecamatan
                </div>
                <div class="flex justify-center sm:justify-end">
                    {{ $kecamatans->onEachSide(1)->links('components.pagination.rounded') }}
                </div>
            </div>
        @endif
    </section>
