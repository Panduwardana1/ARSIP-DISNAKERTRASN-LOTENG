...
@section('pageTitle', 'SIREKAP - PASMI | Destinasi | Ubah')
@section('titleContent', 'Ubah Data Destinasi')
    ...
    <div class="h-full overflow-y-auto py-4">
        <div class="mx-auto max-w-3xl space-y-6 px-2 font-inter">
            <div class="flex flex-col justify-between gap-4 md:flex-row md:items-start">
                <div class="space-y-1">
                    <h2 class="text-xl font-semibold text-zinc-800">Perbarui Negara Tujuan</h2>
                    <p class="text-sm text-zinc-500">Pastikan data destinasi selalu akurat pada modul SIREKAP PASMI.</p>
                </div>
            ...
            <form action="{{ route('sirekap.destinasi.update', $destinasi) }}" method="POST"
                ...
                <div class="space-y-10 p-6 md:p-8">
                    <section class="space-y-6">
                        <div>
                            <h3 class="text-lg font-semibold text-zinc-800">Identitas Destinasi</h3>
                            <p class="text-sm text-zinc-500">Perbarui informasi utama negara tujuan.</p>
                        </div>

                        <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                            <label class="block text-sm font-medium text-zinc-700">
                                Nama Negara <span class="text-rose-500">*</span>
                                <input type="text" name="nama" value="{{ old('nama', $destinasi->nama) }}" maxlength="150"
                                    required
                            ...
                            <label class="block text-sm font-medium text-zinc-700">
                                Kode Negara (ISO Alpha-3) <span class="text-rose-500">*</span>
                                <input type="text" name="kode" value="{{ old('kode', $destinasi->kode) }}" maxlength="3"
                                    required
                                    class="mt-1 w-full rounded-md border border-zinc-200 bg-white px-4 py-2.5 text-sm uppercase tracking-widest text-zinc-700 transition focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/30">
                                @error('kode')
                                    <span class="mt-1 block text-xs text-rose-600">{{ $message }}</span>
                                @enderror
                            </label>
                        ...
                        <div class="grid grid-cols-1 gap-5">
                            <label class="block text-sm font-medium text-zinc-700">
                                Benua
                                <input type="text" name="benua" value="{{ old('benua', $destinasi->benua) }}" maxlength="100"
                                    class="mt-1 w-full rounded-md border border-zinc-200 bg-white px-4 py-2.5 text-sm text-zinc-700 transition focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/30">
                                @error('benua')
                                    <span class="mt-1 block text-xs text-rose-600">{{ $message }}</span>
                                @enderror
