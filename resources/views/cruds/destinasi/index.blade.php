...
@section('titleContent', 'Destinasi')

@section('content')
    {{-- Title Halaman --}}
@section('titlePage', 'Destinasi')
...
<div class="flex h-full flex-col font-inter">
    ...
    <main class="flex-1 px-4 py-6">
        <div class="items-start pb-4 w-full">
            <form method="GET" action="{{ route('sirekap.destinasi.index') }}" class="relative w-full max-w-md" role="search">
            ...
            <div class="rounded-xl border border-zinc-200 bg-white shadow-sm">
                            ...
                            <tbody class="divide-y divide-zinc-100">
                                @foreach ($destinasis as $destinasi)
                                    <tr class="transition hover:bg-zinc-50/70">
                                        ...
                                        <td class="px-6 py-4 align-top">
                                            <span class="rounded-md bg-sky-100 px-3 py-1 text-xs font-semibold uppercase text-sky-700">
                                                {{ $destinasi->kode ?? '-' }}
                                            </span>
                                        </td>
                            ...
                            <article class="rounded-xl border border-zinc-200 p-4 shadow-sm">
                                <div class="flex items-start justify-between gap-3">
                                    <div>
                                        ...
                                        <p class="text-xs text-zinc-500">Kode: <span class="font-semibold uppercase text-zinc-700">{{ $destinasi->kode ?? '-' }}</span></p>
                                    </div>
                ...
                @else
                    <div class="px-6 py-12 text-center text-sm text-zinc-500">
                        ...
                        <a href="{{ route('sirekap.destinasi.create') }}" class="ml-1 font-medium text-emerald-600 underline underline-offset-4">
                            Tambah sekarang
                        </a>
                    </div>
                @endif
