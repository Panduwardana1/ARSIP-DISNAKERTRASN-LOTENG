...
<div class="leading-normal tracking-normal">
    <div class="relative min-h-screen" x-data="{ sidebarOpen: true }">
        {{-- TODO Sidebar --}}
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div x-data="{ open: false }" class="ml-auto relative" x-show="sidebarOpen" x-cloak>
                        <button @click="open = !open" @keydown.escape.window="open = false"
                            class="p-1 rounded-full focus:outline-none" :aria-expanded="open.toString()"
                            aria-haspopup="true">
                        ...
                        <div x-show="open" x-transition @click.outside="open = false"
                            class="absolute right-0 bottom-12 w-40 bg-white border rounded-lg shadow-md py-1"
                            role="menu" aria-orientation="vertical" tabindex="-1">
                            <a href="{{ route('disnakertrans.dashboard') }}"
                                class="block px-3 py-2 text-sm font-inter text-neutral-700 hover:bg-neutral-100"
        ...
        <div class="flex-1 ease-in-out duration-200" :class="{ 'ml-56': sidebarOpen, 'ml-16': !sidebarOpen }">
            {{-- TODO Header --}}
            <header class="items-center px-2 py-4 border-b">
                <div class=" flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    <div class="space-y-0">
                        <h1 class="text-2xl sm:text-2xl pl-4 font-semibold font-inter text-neutral-800">
                            {{ $titlePageContent }}
                        </h1>

                        @if ($subtitle)
                            <p class="text-sm font-inter text-neutral-500">{{ $subtitle }}</p>
                        @endif
            ...
            <main class="bg-neutral-100 max-w-full w-full">
                {{ $slot }}
            </main>
        </div>
    </div>
